if (!isServer) exitWith {}; // Garantir que o script seja executado apenas no servidor
...
// Ensure the script runs only once per master
if (!isNil {_master getVariable "hasDog"}) exitWith {};

// Set variable to indicate the master already has a dog
_master setVariable ["hasDog", true, true];
                ...
                [_dog, _master] spawn
                    {
                        ...
                        while {sleep 1; (alive _dog && alive _master)} do
                        {
                            ...
                            _soundToPlay = _soundPath + (selectRandom ["sounds\dog_bark1.ogg","sounds\dog_bark2.ogg","sounds\dog_bark3.ogg","sounds\dog_pant.ogg","sounds\dog_whimper.ogg"]);
                            private _volume = 3;
                        ...
                        while {sleep 3; (alive _dog && alive _master)} do
                            {
                                ...
                                if (_dog distance _dogDestination > 9) then {_dog playMove "Dog_Sprint"; _dog moveTo _dogDestination; if (random 100 > 75) then {playSound3D [([str missionConfigFile, 0, -15] call BIS_fnc_trimString) + (selectRandom ["sounds\dog_bark1.ogg","sounds\dog_bark2.ogg","sounds\dog_bark3.ogg"]), _dog, false, getPos _dog, 5, 1, 25]}};
                            };
